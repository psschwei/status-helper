...
from status_assistant.models import Issue, IssueAssignee, PullRequest
...
def sync_repository(
    ...
    # not a Python bool.
    #
    # Assignee rows carry no ``repository_id``, so scope their delete to this repo via a
    # subquery on its issue ids — and run it *before* the issues are deleted, while those
    # ids still resolve.
    session.exec(
